...
| SSRF through provider/webhook configuration | Destinations are server-side environment configuration, screening requires HTTPS by default, and callers cannot supply URLs | Allow-list approved hosts and enforce egress policy in the deployment network |
| Malicious or compromised screening provider | Strict response schema, size/candidate bounds, TLS 1.2+, optional private CA/mTLS, timeouts and circuit breaker | Vendor due diligence, credential rotation, contractual incident notification and independent coverage validation |

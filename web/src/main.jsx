...
function App() {
  ...
  const [notificationPreference, setNotificationPreference] = useState({email_address:'',email_enabled:false})
  ...
  const load = async () => {
    ...
    try {
      const [customerPage, alertPage, casePage, notificationPage, preference] = await Promise.all([
        api('/v1/customers?page_size=100', token), api('/v1/alerts?page_size=100', token), api('/v1/cases?page_size=100', token), api('/v1/notifications', token), api('/v1/notification-preferences', token),
      ])
      setCustomers(customerPage.items || []); setAlerts(alertPage.items || []); setCases(casePage.items || []); setNotifications(notificationPage.items || []); setNotificationPreference(preference)
...
function Content({view, customers, alerts, cases, notifications, notificationPreference, token, role, reload}) {
  ...
  if (view === 'notifications') return <NotificationInbox items={notifications} preference={notificationPreference} token={token} reload={reload}/>
...
function NotificationInbox({items,preference,token,reload}){const [email,setEmail]=useState(preference.email_address||'');const [enabled,setEnabled]=useState(preference.email_enabled||false);useEffect(()=>{setEmail(preference.email_address||'');setEnabled(preference.email_enabled||false)},[preference]);const markRead=async id=>{await api(`/v1/notifications/${id}/read`,token,{method:'POST'});await reload()};const save=async e=>{e.preventDefault();await api('/v1/notification-preferences',token,{method:'PUT',body:JSON.stringify({email_address:email,email_enabled:enabled})});await reload()};return <><section className="panel"><div className="panel-title"><div><p className="eyebrow">Delivery preferences</p><h2>Email alerts</h2></div></div><form className="preference-form" onSubmit={save}><label>Email address<input type="email" value={email} onChange={e=>setEmail(e.target.value)} required={enabled}/></label><label className="checkbox"><input type="checkbox" checked={enabled} onChange={e=>setEnabled(e.target.checked)}/>Send screening-match alerts by email</label><button type="submit">Save preferences</button></form></section><section className="panel"><div className="panel-title"><div><p className="eyebrow">Operational notifications</p><h2>Compliance inbox</h2></div><span>{items.filter(item=>!item.read).length} unread</span></div>{items.length?<div className="notification-list">{items.map(item=><article className={item.read?'read':''} key={item.id}><div><strong>{item.title}</strong><p>{item.message}</p><small>{new Date(item.created_at).toLocaleString()}</small></div>{item.read?<span>Read by {item.read_by}</span>:<button className="compact" onClick={()=>markRead(item.id)}>Mark read</button>}</article>)}</div>:<Empty text="No notifications" detail="New screening matches will appear here automatically."/>}</section></>}
...
function ScreeningWorkspace({customers, token, role}) {
  const [customerID,setCustomerID]=useState(customers[0]?.id||'');const[matches,setMatches]=useState([]);const[schedule,setSchedule]=useState(null);const[loading,setLoading]=useState(false);const[error,setError]=useState('')
  const canRun=role==='analyst'||role==='admin';const canReview=role==='reviewer'||role==='admin'
  const load=async()=>{if(!customerID)return;setError('');try{const page=await api(`/v1/customers/${customerID}/screening-matches`,token);setMatches(page.items||[])}catch(e){setError(e.message)}try{setSchedule(await api(`/v1/customers/${customerID}/screening-schedule`,token))}catch{setSchedule(null)}}
  useEffect(()=>{load()},[customerID,token])
  ...
  const configure=async()=>{const hours=Number(prompt('Repeat screening every how many hours?',schedule?.interval_hours||720));if(!Number.isInteger(hours)||hours<1||hours>8760)return;await api(`/v1/customers/${customerID}/screening-schedule`,token,{method:'PUT',body:JSON.stringify({enabled:true,interval_hours:hours})});await load()}
  const toggle=async()=>{await api(`/v1/customers/${customerID}/screening-schedule`,token,{method:'PUT',body:JSON.stringify({enabled:!schedule.enabled,interval_hours:schedule.interval_hours})});await load()}
  ...
  return <section className="screening-page"><section className="screening-hero panel"><div><p className="eyebrow">Financial crime intelligence</p><h2>Sanctions, PEP & adverse media screening</h2><p>Manual review and scheduled ongoing monitoring with explainable matching.</p></div><div className="screening-controls"><select value={customerID} onChange={event=>setCustomerID(event.target.value)}>{customers.map(customer=><option key={customer.id} value={customer.id}>{customer.legal_name}</option>)}</select>{canRun&&<button disabled={loading} onClick={run}>{loading?'Screening…':'Run screening'}</button>}</div></section>{error&&<div className="error">{error}</div>}<section className="panel monitoring-schedule"><div><p className="eyebrow">Ongoing monitoring</p><h2>{schedule?(schedule.enabled?'Automatic screening active':'Automatic screening paused'):'No schedule configured'}</h2><p>{schedule?`Every ${schedule.interval_hours} hours · next ${new Date(schedule.next_run_at).toLocaleString()}${schedule.last_error?` · last error: ${schedule.last_error}`:''}`:'Configure recurring screening for this customer.'}</p></div>{canRun&&<div>{schedule&&<button className="secondary" onClick={toggle}>{schedule.enabled?'Pause':'Resume'}</button>}<button onClick={configure}>{schedule?'Change cadence':'Enable monitoring'}</button></div>}</section><section className="customer-metrics screening-metrics"><article><span>Total matches</span><strong>{matches.length}</strong><small>All screening history</small></article><article><span>Potential</span><strong>{potential}</strong><small>Requires human review</small></article><article><span>Confirmed</span><strong>{matches.filter(item=>item.status==='confirmed').length}</strong><small>True matches</small></article><article><span>False positive</span><strong>{matches.filter(item=>item.status==='false_positive').length}</strong><small>Disposition recorded</small></article></section><section className="panel"><div className="panel-title"><div><p className="eyebrow">Match review queue</p><h2>Screening results</h2></div><span>{potential} awaiting disposition</span></div>{matches.length?<div className="match-list">{matches.map(match=><article key={match.id}><div className="match-score"><strong>{match.score}</strong><span>match</span></div><div><h3>{match.query_name} ↔ {match.matched_name}</h3><p>{match.subject_type.replaceAll('_',' ')} · {match.reason}</p><div>{badge(match.list_type)} {badge(match.status)}</div></div><div className="match-review">{match.reviewed_by?<><strong>{match.reviewed_by}</strong><span>{match.disposition_reason}</span></>:canReview&&match.status==='potential'?<><button onClick={()=>disposition(match,'confirmed')}>Confirm</button><button className="reject" onClick={()=>disposition(match,'false_positive')}>False positive</button></>:<span>Review required</span>}</div></article>)}</div>:<Empty text="No screening matches" detail="Run screening to check the customer and beneficial owners."/>}</section></section>
}
